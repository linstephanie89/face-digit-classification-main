...
class Counter(dict):
    ...
    def argMax(values):
        ...
        if len(values) == 0:
            return -1
        # Convert the dict_items object to a list
        all = list(values.items())
        maxIndex = 0
        for i in range(1, len(all)):
            if all[i][1] > all[maxIndex][1]:
                maxIndex = i
...
def lookup(name, namespace):
    ...
    if dots > 0:
        ...
        if len(options) == 1:
            return options[0]
        if len(options) > 1:
            raise Exception(print('Name conflict for %s'))
        raise Exception(print('%s not found as a method or class' % name))
